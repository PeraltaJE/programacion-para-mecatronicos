...
int main(void)
{
    int VOT, C1 = 0, C2 = 0, C3 = 0, C4 = 0, C5 = 0, NU = 0, SVO;// Inicialización de variables para contar votos.
    float PO1, PO2, PO3, PO4, PO5, PON;// Variables para almacenar porcentajes de votos.
    printf("Ingrese el primer voto: ");
    scanf("%d", &VOT);
    while (VOT)
    {
        switch(VOT)//aumenta el contador en cada candidato y nulo
        {
        case 1:
            C1++;
            break;
    ...
    SVO = C1 + C2 + C3 + C4 + C5 + NU;//calcula la cantidad de votos
    PO1 = ((float) C1 / SVO) * 100;
    PO2 = ((float) C2 / SVO) * 100;
    PO3 = ((float) C3 / SVO) * 100;//calcula el porcentaje de los los votos
